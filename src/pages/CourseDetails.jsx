...
export default function CourseDetails() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log("Hello",res?.data.totalDuration)
        if (res && res.success) {
          // setResponse(response => response =  res.data[0]);
          setTotalDuration(res?.data.totalDuration)
          setResponse(res?.data.courseDetails[0]);
         // console.log(response)
          // console.log("course details res: ", response);
        }
        // ✅ SET THE ENTIRE COURSE OBJECT
      } catch (error) {
  ...
  // console.log(response)
  useEffect(() => {
    if (!response) return; // Guard clause
  ...
  const whatYouWillLearn = response.whatYouWillLearn;
  const courseContent = response.courseContent;
  const ratingAndReviews = response.ratingAndReviews;
  const instructor = response.instructor;
  const studentsEnrolled = response.studentsEnrolled;
